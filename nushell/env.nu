export-env {
  ...
  let brew_sbin = "/opt/homebrew/sbin"
  ...
  load-env {
    PATH: (
      ...
      | prepend ($gopath | path join "bin")
      | prepend ($home | path join ".config" "carapace" "bin")
      | prepend ($home | path join ".spicetify")
      | prepend ($pnpm_home)
      | prepend ($home | path join ".scripts")
    ...
    XDG_RUNTIME_DIR: ("/tmp/user-" + ((id -u) | into string))

    # User dirs
    XDG_DESKTOP_DIR: ($home | path join "Desktop")
    XDG_DOWNLOAD_DIR: ($home | path join "Downloads")
    XDG_TEMPLATES_DIR: ($home | path join "Templates")
    XDG_PUBLICSHARE_DIR: ($home | path join "Public")
    XDG_DOCUMENTS_DIR: ($home | path join "Documents")
    XDG_MUSIC_DIR: ($home | path join "Music")
    XDG_PICTURES_DIR: ($home | path join "Pictures")
    XDG_VIDEOS_DIR: ($home | path join "Videos")
